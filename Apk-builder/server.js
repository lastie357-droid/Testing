...
function finishJob(jobId, status, error) {
  ...
  if (state.recentJobs.length > MAX_RECENT_JOBS)
    state.recentJobs.length = MAX_RECENT_JOBS;
  delete state.currentJobs[jobId];
  if (
    Object.keys(state.currentJobs).length === 0 &&
...
function parseLine(raw) {
  ...
  if (m) {
    finishJob(m[1], "success", null);
    return;
  }

  m = line.match(/❌\s*Job\s+(\S+)\s+failed\s+—\s+(.+)$/);
  if (m) {
    finishJob(m[1], "failed", m[2].trim());
    return;
  }
}
...
function startWorker() {
  ...
  if (!BUILD_URL_EXPLICIT) {
    state.workerStatus = "misconfigured";
    pushLog(
      `[worker not started: BUILD_URL not set — auto-derived to ${BUILD_URL} which is this worker itself. Set BUILD_URL to your dashboard URL.]`,
    );
    return;
  }
...
function renderPage() {
  const uptimeMs = state.workerStartedAt
  ...
  const curList = Object.values(state.currentJobs).sort(
